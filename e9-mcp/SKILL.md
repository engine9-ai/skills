...
| List or run plugin-installed reports (filters, date range) | `report` (`list` first; then `run` with `path` + options) |
...
List, load, or run **plugin-installed** reports for an account. Reports are not hardcoded — they come from installed plugins (`<package>:reports:<key>`). SQL is generated by `ReportWorker`. Full authoring + UI widget contract: [e9-reports](../e9-reports/SKILL.md).

- Required: `account_id`
- **command: list** (default) — catalog: `path`, `name`, `description`, `tags`, `plugin.instances`, **`filters`** (JSON Schema), component summaries
- **command: get** — definition without running. Requires `path`
- **command: run** — execute `path` with `options` / `start` / `end` / `limit` / `offset`. Returns component `data` plus top-level `sql`
...
HTTP: `GET /data/reports`, `GET /data/reports/get?path=`, `GET|POST /data/reports/run`.
