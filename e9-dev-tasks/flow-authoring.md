...
Flow definition files can live **anywhere** — pass the path to `createFlowRun({ flow: '/any/path/to/flow.json5', ... })` or `loadFlowFromFile`. `listFlows` / `getFlowById` scan the account flows directory (`flowsDir`, `{ENGINE9_ACCOUNT_DIR}/{account_id}/flows`, or `TASK_API_FLOWS_DIR`) **plus** built-in server flows (`server/flows/*.json5`, `server/rebuild/*.flow.json5`). Relative `flow` / `flow_path` values resolve from the **server package root** first (not process cwd). See [e9-tasks-api/deploy-flow.md](../e9-tasks-api/deploy-flow.md).
