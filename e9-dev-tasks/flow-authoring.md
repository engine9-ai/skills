...
This document is based on `manager/TaskWorker.js`, `manager/SQLTaskManager.js`, and the examples in `engine9/server/test/task/*.json5`. The Prefect-compatible REST routes for these flows are mounted on the **main API** (`api/index.js` via `api/mcp/mount.js`) so they share the same Google OAuth pipeline as `/mcp` — see [e9-tasks-api README](../e9-tasks-api/README.md) (API consumers) and `engine9/server/api/task/docs/admin/README.md` (operators).
